...
import SelectPaging from "../SelectPaging/SelectPaging";
...
function SelectSearch(contentProps: SelectContentProps) {
  ...
  const handleSearch = async (queryParams: AccountPublicSearchQueryModel) => {
    setIsSearching(true);
  ...
  const handleBlur = async () => {
    ...
    if (res) {
      setData((prevState) => {
        return {
          ...prevState,
          metadata: { ...res.metadata, page: 1 },
          items: initialAuditedBy.length > 0 ? [...initialAuditedBy, ...res.items] : res.items,
        };
      });
    }
  };
